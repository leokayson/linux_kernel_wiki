...
~~多线程使用的基本是POSIX标准提供的接口函数，而多进程则是基于System V。在信号量这种常用的同步互斥手段方面，POSIX在无竞争条件下是不会陷入内核的，而SystemV即无论何时都会陷入内核。这就给多线程每次调用都会陷入内核，丧失了线程的轻量优势。所以多线程之间的通信不是用System V。~~

| 特性   | POSIX信号量         | System V信号量  |
| ---- | ---------------- | ------------ |
| 标准   | POSIX            | System V     |
| 无竞争时 | **用户空间操作，不陷入内核** | **必须陷入内核**   |
| 适用场景 | 多线程、轻量级同步        | 多进程、复杂同步需求   |
| 性能   | 更高（无竞争时）         | 较低（总有系统调用开销） |

| 场景      | 主要使用的接口                                | 说明                          |
| ------- | -------------------------------------- | --------------------------- |
| **多线程** | **NPTL (Native POSIX Thread Library)** | Linux 的线程实现本身就是基于 POSIX 标准的 |
| **多进程** | POSIX 或 System V 均可                    | 但现代代码更倾向于 POSIX             |
