...
# 所有缓存分类
缓存种类：
 高速缓存（Cache）是计算机系统中用于**临时存储高频访问数据**的高速存储层，通过减少数据访问延迟来提升性能。以下是主要分类及其作用：
...
| 类型 | 位置 | 速度 | 容量 | 作用 |
|------|------|------|------|------|
| **L1 Cache（一级缓存）** | CPU核心内部 | 最快（~1ns） | 32-128KB | 存储最频繁的指令和数据，每个核心独立拥有 |
| **L2 Cache（二级缓存）** | CPU核心内部/共享 | 较快（~3-10ns） | 256KB-1MB | 作为L1的后备，容量更大，部分架构为每个核心独享 |
| **L3 Cache（三级缓存）** | CPU多核心共享 | 较快（~10-20ns） | 4-64MB | 多核心间共享数据，减少内存访问冲突 |
| **主存（RAM）** | 主板内存条 | 慢（~100ns） | 8-128GB | 运行中的程序和数据 |
| **磁盘缓存** | 磁盘控制器/SSD DRAM | 较慢（~1ms SSD / ~10ms HDD） | 8MB-1GB+ | 磁盘内置缓存，加速读写操作 |
...
### 1. **CPU缓存（处理器缓存）**
- **作用**：缓解CPU与内存之间的速度鸿沟（CPU比内存快100倍以上）
- **工作原理**：利用**时间局部性**（刚访问的数据可能再次访问）和**空间局部性**（相邻数据可能被一起访问）
- **示例**：Intel的Smart Cache、AMD的Game Cache

### 2. **磁盘缓存（Disk Cache/Buffer）**
- **作用**：减少磁盘I/O次数，合并小写入操作
- **位置**：
  - 机械硬盘（HDD）：8-256MB DRAM缓存
  - 固态硬盘（SSD）：SLC缓存、DRAM缓存（部分无缓盘用HMB技术借用系统内存）
- **风险**：掉电可能导致数据丢失（需配合电容保护）

### 3. **页缓存（Page Cache）**
- **位置**：操作系统内核管理的内存区域
- **作用**：缓存文件系统的数据页，加速文件读写
- **Linux命令**：`free -h` 显示的 `buff/cache` 即包含此部分

### 4. **Web缓存**
| 类型 | 层级 | 作用 |
|------|------|------|
| **浏览器缓存** | 客户端 | 缓存HTML、CSS、JS、图片，减少重复下载 |
| **CDN缓存** | 边缘节点 | 分布式部署，就近响应用户请求 |
| **反向代理缓存**（如Nginx、Varnish） | 服务端 | 缓存动态内容，减轻应用服务器压力 |
| **数据库查询缓存** | 数据库层 | 缓存SQL查询结果 |

### 5. **数据库缓存**
- **缓冲池（Buffer Pool）**：如MySQL InnoDB缓存表数据和索引
- **查询缓存**：缓存SQL语句的结果（MySQL 8.0已移除）
- **连接池**：缓存数据库连接，减少创建开销

### 6. **应用程序缓存**
- **本地缓存**：如Guava Cache、Caffeine（Java），进程内高速访问
- **分布式缓存**：如Redis、Memcached，多服务共享，支撑高并发
...
| 策略 | 描述 | 适用场景 |
|------|------|----------|
| **直写（Write-through）** | 数据同时写入缓存和持久存储 | 数据一致性要求高 |
| **回写（Write-back）** | 数据先写缓存，延迟异步刷盘 | 性能优先，有数据丢失风险 |
| **写直达（Write-around）** | 绕过缓存直接写存储，仅读时缓存 | 写多读少场景 |
...
```
数据流动：CPU → L1 → L2 → L3 → 内存 → 磁盘缓存 → 磁盘
         ↑      ↑     ↑     ↑      ↑       ↑        ↑
       最快   很快   较快  快      慢      较慢     最慢
```

> **核心思想**：用成本更高、速度更快的存储，暂存最可能被访问的数据，以**空间换时间**。
