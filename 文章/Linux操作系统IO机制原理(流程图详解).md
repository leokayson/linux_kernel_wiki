...
| 方式                      | 核心机制                        | 地址空间              | 典型应用场景              |
| :---------------------- | :-------------------------- | :---------------- | :------------------ |
| **端口映射 I/O (PMIO/PIO)** | 专用 I/O 指令（IN/OUT）访问独立端口空间   | 独立 64KB 端口空间（x86） | 传统 x86 外设（如早期串口、并口） |
| **内存映射 I/O (MMIO)**     | 普通访存指令访问设备寄存器               | 与内存共享地址空间         | 现代主流方式（PCIe、显卡、网卡等） |
| **DMA（直接内存访问）**         | 专用 DMA 控制器接管总线，直接在内存↔设备间传数据 | 不经过 CPU 寄存器       | 磁盘、网络、显卡等高吞吐设备      |
| **通道 I/O / I/O 处理器**    | 专用 I/O 处理器独立执行通道程序          | 独立子系统             | 大型机（IBM 大型机）、高端存储系统 |

---

### 二、详细对比与区别

#### 1. 端口映射 I/O（Programmed I/O / Port I/O）

```
CPU ──IN/OUT指令──→ I/O 端口 ──→ 设备
     (如: IN AL, 0x60)
```

|特性|说明|
|:--|:--|
|**指令隔离**|必须专用 IN/OUT 指令，与普通内存访问区分|
|**地址空间**|x86 有独立 16 位端口空间（0-65535）|
|**优势**|不会污染内存地址空间，I/O 操作明确可追踪|
|**劣势**|需要专用指令，灵活性差，性能较低|
|**现状**|x86 保留兼容，现代设备已很少使用|
...
```
CPU ──MOV指令──→ 内存地址空间 ──→ 映射到设备寄存器/显存
     (如: MOV [0xFE000000], EAX)
```

| 特性       | 说明                                  |
| :------- | :---------------------------------- |
| **访问方式** | 普通访存指令（MOV/LDR/STR 等）               |
| **地址空间** | 与 RAM 共享物理地址空间（需预留区域）               |
| **优势**   | 指令集统一，可用所有寻址模式，Cache 可控（标记为设备内存）    |
| **劣势**   | 占用内存地址空间，需防止 Cache 一致性问题            |
| **现状**   | **现代绝对主流**（PCIe、ARM SoC、GPU、NVMe 等） |
...
```
       ┌─────────────────────────────┐
       ↓                             │
CPU → 配置DMA控制器 → 设备 ↔ DMA ↔ 内存
       (源/目的/长度)    (脱离CPU传输)
```

|特性|说明|
|:--|:--|
|**核心思想**|CPU 初始化后，**数据传输不经过 CPU**|
|**工作方式**|DMA 控制器申请总线控制权，直接搬运数据|
|**优势**|**解放 CPU**，适合大数据块传输，吞吐极高|
|**类型**|第三方 DMA（独立芯片）→ 现代演变为 **总线主控（Bus Mastering）**|
|**典型应用**|NVMe SSD、千兆/万兆网卡、显卡显存交换、磁盘 RAID|

---

#### 4. 通道 I/O / I/O 处理器（Channel I/O）


```
CPU ──简单命令──→ 通道处理器 ──执行复杂通道程序──→ 多个设备
     (启动I/O)      (专用小CPU)
```

|特性|说明|
|:--|:--|
|**核心思想**|专用 I/O 处理器（通道）**自主执行 I/O 程序**|
|**CPU 负担**|极简，只需发送"启动通道"命令|
|**通道程序**|通道可独立执行复杂操作（寻道、传输、链式操作）|
|**优势**|CPU 完全解放，可管理大量并发 I/O|
|**典型系统**|**IBM 大型机（zSeries）**、高端存储阵列|
|**与 DMA 区别**|通道是"可编程的 DMA"，能执行复杂逻辑，非单纯数据搬运|
...
│  软件控制程度 ←────────────────────────────→ 硬件自主程度      │
├───────────────┬───────────────┬──────────────────┬──────────┤
│   PIO (CPU    │  MMIO (CPU    │   DMA (控制器    │ 通道/IOP │
│   逐指令控制)  │   逐指令控制)  │   自动搬运数据)   │ (自治执行)│
├───────────────┴───────────────┴──────────────────┴──────────┤
│  性能/吞吐量  ──────────────────────────────────────→  高     │
│  CPU 占用    高 ←────────────────────────────────→  极低    │
│  灵活性      低 ←────────────────────────────────→  高      │
...
| 层次        | 机制                     | 例子                     |
| :-------- | :--------------------- | :--------------------- |
| **寄存器访问** | MMIO                   | PCIe 配置空间、设备控制寄存器      |
| **数据传输**  | DMA（Bus Mastering）     | NVMe SSD 读写到内存，网卡收发包   |
| **中断通知**  | MSI/MSI-X（基于 MMIO 的中断） | 设备完成 DMA 后通过内存写通知 CPU  |
| **大数据流**  | RDAM / GPU Direct      | 绕过 CPU 内存，设备间直接 P2P 传输 |

**现代趋势**：MMIO + DMA 为主流，PIO 仅 x86 遗留兼容，通道 I/O 限于特定高端场景。