...
[ 页目录索引 10位 | 页表索引 10位 | 页内偏移 12位 ]
        ↓                    ↓              ↓
   定位页目录条目      定位页表条目      定位物理字节
```
页目录表（Page Directory Table, PDT）

|属性|说明|
|:--|:--|
|**本质**|二级页表结构中的第一级，每个条目指向一个页表|
|**位置**|CPU的CR3寄存器存储其物理地址|
|**条目数**|1024个条目（32位系统），每个4字节，共4KB|
|**覆盖范围**|每个条目管理4MB内存，整个目录管理4GB虚拟地址空间|
页表（Page Table）

|属性|说明|
|:--|:--|
|**本质**|二级页表结构中的第二级，每个条目指向物理页框|
|**条目数**|1024个条目，每个4字节，共4KB（一页大小）|
|**管理粒度**|每个条目对应4KB物理页框|
|**数量**|由页目录决定，最多1024个页表|
缓冲区（Buffer）
...
|属性|说明|
|:--|:--|
|**本质**|内存中临时存储数据的区域，用于协调不同速度的设备|
|**位置**|物理内存、内核空间、用户空间均可存在|
|**管理单位**|块（Block，通常512B-4KB）或页（Page）|
