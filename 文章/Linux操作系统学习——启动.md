...
| 起始地址       | 结束地址       | 用途                               | 大小           | 详细用途说明（中文）                                                                                                         |
| ---------- | ---------- | -------------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------ |
| 0x00000000 | 0x000003FF | Real Mode Interrupt Vector Table | **1 KB**     | **实模式中断向量表**：包含256个中断向量（每个4字节，格式为CS:IP），用于存储中断处理程序的入口地址。前32个保留给CPU异常，其余供BIOS和硬件中断使用。系统启动时由BIOS初始化，是CPU响应中断的关键数据结构。 |
| 0x00000400 | 0x000004FF | BIOS Data Area                   | **256 B**    | **BIOS数据区**：存储系统硬件配置和状态信息，包括串口/并口基地址、显示器模式、内存容量、键盘状态标志、磁盘驱动器数量、实时时钟数据等。BIOS和操作系统通过此区域获取硬件参数。                       |
| 0x00000500 | 0x00007BFF | Unused                           | **30.25 KB** | **常规内存空闲区**：实模式下可供操作系统和应用程序使用的空闲内存。早期DOS系统会将自身和设备驱动程序加载至此。空间较小，通常用于临时数据存储或栈空间。                                     |
| 0x00007C00 | 0x00007DFF | Our Bootloader                   | **512 B**    | **引导扇区加载区**：BIOS从启动设备读取第一个扇区（512字节）并加载到此固定地址。这是x86架构的标准引导入口点，包含引导代码和分区表信息。操作系统由此开始接管系统控制权。                         |
| 0x00007E00 | 0x0009FFFF | Unused                           | **590.5 KB** | **扩展常规内存**：大量的可用内存空间，操作系统内核、驱动程序、文件系统缓存通常加载至此。实模式下可寻址的最大内存区域（约608KB），是系统启动后的主要工作区域。                                |
| 0x000A0000 | 0x000BFFFF | Video RAM (VRAM)                 | **128 KB**   | **图形显存区**：VGA/EGA/CGA显卡使用的帧缓冲区，用于存储图形像素数据。支持图形模式（如320x200、640x480等），CPU直接读写此区域来控制屏幕显示内容，是最早的共享内存架构实例。              |
| 0x000B0000 | 0x000B7777 | Monochrome Video Memory          | **~30 KB**   | **单色文本显存**：MDA（单色显示适配器）专用，用于黑白文本模式显示。内存映射为字符和属性字节对，支持80x25文本模式。主要用于早期服务器或调试终端的文本显示。                                |
| 0x000B8000 | 0x000BFFFF | Color Video Memory               | **32 KB**    | **彩色文本/图形显存**：CGA及以上显卡使用，彩色文本模式的默认帧缓冲区。每个字符由ASCII码和属性字节（前景色、背景色、闪烁位）组成，支持16色文本显示，也是部分图形模式的数据区。                     |
| 0x000C0000 | 0x000C7FFF | Video ROM BIOS                   | **32 KB**    | **显卡BIOS区**：存储显功能、VESA扩展功能等。系统启动时CPU执行此区域代码来初始化显示设备，建立基本的视频服务中断（INT 10h）。卡固件，包含显卡初始化代码、基本图形/文本输出                   |
| 0x000C8000 | 0x000EFFFF | BIOS Shadow Area                 | **160 KB**   | **扩展BIOS/影子内存**：用于映射其他硬件设备（网卡、SCSI卡、声卡等）的ROM，或作为上位内存块（UMB）。可配置复制到RAM（Shadowing）以提高访问速度，释放后可用作常规内存。                 |
| 0x000F0000 | 0x000FFFFF | System BIOS                      | **64 KB**    | **主板系统BIOS**：包含加电自检（POST）程序、系统启动引导代码、CMOS设置程序、硬件中断服务（INT 13h磁盘服务等）。是主板固件核心，负责硬件初始化和操作系统引导，通常映射自ROM芯片。              |
